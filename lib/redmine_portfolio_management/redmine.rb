module PortfolioManagement
    ...
    module Redmine
        ...
        def plugin
            ::Redmine::Plugin.find(:portfolio_management)
        end

        def settings
            if ActiveRecord::Base.connection.table_exists?(:settings) && self.plugin && Setting.plugin_portfolio_management
                Setting.plugin_portfolio_management
            else
